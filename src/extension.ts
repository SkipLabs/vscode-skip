...
const documentSymbolProvider = {
  provideDocumentSymbols(
    ...
    const symbolRE =
      /(?<!\/\/\s*)(?:\b(?<kind1>class|const|fun|module|trait|type)\s+\.?(?<name1>\w+)|(?<kind2>\|)\s*\.?(?<name2>\w+)\s*\((?!.+->))/dg;
    ...
    for (const match of document.getText().matchAll(symbolRE)) {
      ...
      const range = new vscode.Range(
        document.positionAt(nameIndices[0]),
        document.positionAt(nameIndices[1]),
      );
      const location = new vscode.Location(document.uri, range);
      const containerName = currentClass ?? currentModule ?? "";
      results.push(
        new vscode.SymbolInformation(name, kind, containerName, location),
      );
      if (rawKind === "module") {
        currentModule = name;
        currentClass = undefined;
...
const addDefinitionsInDocument = (
  add: (loc: vscode.Location) => boolean,
  document: vscode.TextDocument,
  word: string,
  token: vscode.CancellationToken,
): boolean => {
  const symbolRE = new RegExp(
    `(?<!\\/\\/\\s*)(?:\\b(?:class|const|fun|module|trait|type)\\s+\\.?${word}\\b|\\|\\s*\\.?${word}\\s*\\((?!.+->))`,
    "dg",
  );
  ...
  while ((match = symbolRE.exec(text)) !== null) {
    ...
    if (!add(new vscode.Location(document.uri, pos))) return false;
  }
  return true;
};
...
const documentDefinitionProvider = {
  async provideDefinition(
    ...
    const add = (loc: vscode.Location): boolean => {
      results.push(loc);
    ...
    if (addDefinitionsInDocument(add, document, word, token)) {
      for (const file of files) {
        ...
        if (!addDefinitionsInDocument(add, fileDocument, word, token)) break;
      }
    }
    return results;
  },
...
export function activate(context: vscode.ExtensionContext) {
  ...
  const symbolProviderDisposable =
